...
int flag = 1;
int client_num = 0;
int created_recv_num = 0;		//num of created recv threads.
int created_send_num = 0;		//num of created send threads.

pthread_t thread_accept_id, thread_recv_ids[MAX_CLIENT_NUM], thread_send_ids[MAX_CLIENT_NUM], thread_quit_id, thread_create_recv_id, thread_create_send_id;

void* thread_accept(void *arg);
void* thread_recv(void *arg);
void* thread_send(void *arg);
void* thread_create_recv(void *arg);
void* thread_create_send(void *arg);
...
int main(int argc, char const *argv[])
{
	...
	extern pthread_t thread_accept_id, thread_recv_ids[MAX_CLIENT_NUM], thread_send_ids[MAX_CLIENT_NUM], thread_quit_id, thread_create_recv_id, thread_create_send_id;
	...
	if (pthread_create(&thread_create_recv_id, NULL, thread_create_recv, NULL)) {
		perror("Create create_recv thread error!");
		return 0;
	}

	if (pthread_create(&thread_create_send_id, NULL, thread_create_send, NULL)) {
		perror("Create create_send thread error!");
		return 0;
	}
	...
	if (pthread_join(thread_create_recv_id, NULL)) {
		perror("Wait create_recv thread error!");
		return 0;
	}

	if (pthread_join(thread_create_send_id, NULL)) {
		perror("Wait create_send thread error!");
		return 0;
	}
...
void* thread_create_recv(void *arg) {	//create recv threads
	extern int client_num;
	extern int created_recv_num;
	extern pthread_t thread_recv_ids[MAX_CLIENT_NUM];

	pthread_t thread_recv_id;

	while (1) {
		int client_num_list[client_num];
		if (created_recv_num < client_num) {		// num of created client less than num of accepted client.
			client_num_list[created_recv_num] = created_recv_num;
			thread_recv_id = thread_recv_ids[created_recv_num];
			if (pthread_create(&thread_recv_id, NULL, thread_recv, &client_num_list[created_recv_num])) {
				perror("Create recv subthread error!");
				return 0;
			}else {
				created_recv_num++;
				printf("Create recv subthread %d OK!\n", created_recv_num);
			}
		}
	}
}

void* thread_create_send(void *arg) {	//create send threads
	extern int client_num;
	extern int created_send_num;
	extern pthread_t thread_send_ids[MAX_CLIENT_NUM];

	pthread_t thread_send_id;

	while (1) {
		int client_num_list[client_num];
		if (created_send_num < client_num) {		// num of created client less than num of accepted client.
			client_num_list[created_send_num] = created_send_num;
			thread_send_id = thread_send_ids[created_send_num];
			if (pthread_create(&thread_send_id, NULL, thread_send, &client_num_list[created_send_num])) {
				perror("Create send subthread error!");
				return 0;
			}else {
				created_send_num++;
				printf("Create send subthread %d OK!\n", created_send_num);
			}
		}
	}	
}
...
void* thread_quit(void *arg) {	//quit thread
	extern int flag;
	extern int client_num;
	extern pthread_t thread_accept_id, thread_recv_ids[MAX_CLIENT_NUM], thread_send_ids[MAX_CLIENT_NUM], thread_create_recv_id, thread_create_send_id;
	pthread_t thread_recv_id, thread_send_id;
	int i;
	...
	while(1) {
		if (!flag) {
			...
			printf("Cancel create_recv thread!\n");
			pthread_cancel(thread_create_recv_id);
			printf("Cancel create_send thread!\n");
			pthread_cancel(thread_create_send_id);
