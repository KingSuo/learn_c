...
pthread_t thread_accept_id, thread_recv_id, thread_send_id, thread_quit_id;
	...
	while (1) {
		...
		for (i = 0; i < client_num; i++) {
			if (client_socket_ids[i] == -1) {
				for (j = client_num - 1; j > i; j--) {
					if (client_socket_ids[j] != -1) {
						client_socket_ids[i] = client_socket_ids[j];
						client_num--;
						break;
					}else {
						client_num--;
...
void* thread_quit(void *arg) {	//quit thread
	extern int flag;
	extern pthread_t thread_accept_id, thread_recv_id, thread_send_id;
